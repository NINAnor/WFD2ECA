...
fraVFtilNI(DATA, vannforekomster, vannlokaliteter, parameter, vannkategori,
           ...
           maalingPer = 25, maalingTot = 100, maalingTyp = 25,
           EQR = "asymptotisk",
           ignorerVariabel = NULL, fastVariabel = NULL, 
           aktVekting = TRUE, aktivitetsvekt = 5, 
           antallvekt = 0.5, tidsvekt = 1, arealvekt = 2,
           DeltaAIC = 2, ekstrapolering = "kjente", 
...
- `DATA` (**tabell**). _Argumentet må angis_. Det skal være **R**-tabellen som inneholder de relevante målingene fra vannmiljø-databasen.
- `vannforekomster` (**tabell**). _Argumentet må angis_. Det skal være **R**-tabellen som inneholder informasjonen om vannforekomster.
- `vannlokaliteter` (**tabell**). _Argumentet må angis_. Det skal være **R**-tabellen som inneholder informasjonen om vannlokaliteter.
...
- `paramFil` (**tekst-skalar**). Filnavn på excel-regnearket med informasjon om vannforskriftsparametere. Hvis det ikke er angitt, antas filnavnet å være «VM-param.xlsx».
- `aktivFil` (**tekst-skalar**). Filnavn på excel-regnearket med informasjon om overvåkingsaktiviteter. Hvis det ikke er angitt, antas filnavnet å være «VM-aktiv.xlsx».
...
- `maalingTyp` (**numerisk skalar**). Minste antall målinger som må foreligge for en typologifaktor, for at det gjøres et forsøk på å inkludere typologifaktoren i modellen. Standardinnstillinga er 25.
- `EQR` (**tekst-skalar**). Måten mEQR-verdier (eller nEQR-verdier) blir beregna på. Standardinnstillinga er "asymptotisk". Tilgjengelige alternativ er "forlengelse", "knekk" og "nEQR". (Se [utdypende forklaring](asympEQR.md).)
...
- `aktVekting` (**sannhetsverdi**). Måten vekting for overvåkingsaktivitet gjennomføres på. De to tilgjengelige alternativene gir veldig like resultater. Standardinnstillinga (`aktVekting = TRUE`) bør bare endres for å gjenskape resultater av tidligere versjoner av funksjonen.
...
- `ekstrapolering` (**tekst-skalar**). Ved `ekstrapolering = "kjente"` (standardinnstillinga) ekstrapoleres økologisk tilstand bare til vannforekomster som har en typologi som det faktisk foreligger målinger for. Alternativet er `ekstrapolering = "alle"`, som innebærer at det forsøkes å ekstrapolere økologisk tilstand til alle vannforekomster som parameteren er definert for. (Se [utdypende forklaring](extrapol.md).)
...
- `SEED` (**numerisk skalar**). "Frø" eller "såkorn"" for slumptallgeneratoren. Ifølge standardinnstillinga (`SEED = NULL`) genereres ulike slumptall ved hver kjøring. Ved å angi et heltall i stedet kan man sikre at flere kjøringer benytter de samme slumptallene.
