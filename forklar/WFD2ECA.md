...
- `lokalModell` (**sannhetsverdi-** eller **numerisk skalar**). Argumentet bestemmer måten målte vannforekomster håndteres under simuleringa. Ved `lokalModell = FALSE` (eller 0) blir målte og ekstrapolerte verdier behandla likt (dette var praksis frem til versjon 1.4). Ved `lokalModell = TRUE` (eller 1) blir usikkerheten til målte vannforekomster kvantifisert som konfidensintervaller (mot prediksjonsintervaller for ekstrapolerte vannforekomster), og ¤¤¤
