...
- `trend` (**sannhetsverdi-skalar**). Ved `trend = TRUE` inkluderes en trend for hver rapporteringsperiode i modellen (inkludert interaksjon med rapporteringsperiode). Ved `trend = TRUE` estimeres ikke noen trend innenfor rapporteringsperioder. Ved `trend = NA` (standardinnstillinga) estimeres trender kun om rapporteringsperiodene er lenger enn 3&nbsp;år. (Argumentet ble tilføyd i versjon 2.0.)
