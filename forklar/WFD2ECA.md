...
WFD2ECA(DATA, vannforekomster, vannlokaliteter, parameter, vannkategori,
        ...
        utMaaling = FALSE, utEQR = FALSE, utEstimat = FALSE, 
...
- `rapportenhet` (**tekst-vektor**). Romlig enhet som verdiene skal skaleres opp til. Tillate svar er én eller flere av «vannforekomst», «kommune», «fylke», «landsdel» og «Norge». Standardinnstillinga er å bruke de to siste.
...
- `utEQR` (**sannhetsverdi-skalar**). Ved `utEQR = TRUE` lagres et datasett med målinger før og etter skalering som som en variabel (tabell/dataramme) hvis navn blir oppgitt. (Argumentet ble tilføyd i versjon 2.1.)
- `utEstimat` (**sannhetsverdi-skalar**). Ved `utEstimat = TRUE` lagres et datasett med estimatene for alle relevante vannforekomster og år som en variabel (tabell/dataramme) hvis navn blir oppgitt. (Argumentet ble tilføyd i versjon 2.1.)
...
- `aktivitetsvekt` (**numerisk skalar**). Tallverdi som blir brukt for vekting av overvåkingsaktiviteter. Jo høyere vekt, desto mer vektes overvåkingsaktiviteter med høy «skjevhetsskår» ned ved estimering av verdier. Standardinnstillinga er 2 (men den var 5 frem til versjon 1.4 og 3 fra versjon 1.5 til 2.0), som betyr at aktiviteter får 2 ganger lavere vekt når de har en skår på ±1, 4 ganger lavere vekt når de har en skår på ±2, og 8 ganger lavere vekt når de har en skår på ±3. (Se også argumentet `maksSkjevhet` over, som tillater å ekskludere aktiviteter med visse skjevhetsskår helt, og [utdypende forklaring](aktiv.md).)
...
- `lokalModell` (**sannhetsverdi-** eller **numerisk skalar**). Argumentet bestemmer måten målte vannforekomster håndteres under simuleringa. Ved `lokalModell = FALSE` (eller 0) blir målte og ekstrapolerte verdier behandla likt (dette var praksis frem til versjon 1.4). Hvis `lokalModell` &ge; 0,5, blir usikkerheten til målte vannforekomster kvantifisert som konfidensintervaller fra den respektive vannforekomsten og rapportperioden (mot prediksjonsintervaller for ekstrapolerte vannforekomster; dette var praksis fra versjon 1.5 til 2.0). Ved `lokalModell = TRUE` (eller 1), blir det dessuten tilpassa en lokal tidsavhengig modell i slike tilfeller, gitt at `trend = TRUE`, og at det foreligger nok data fra vannforekomsten i den aktuelle rapportperioden. (Argumentet ble tilføyd i versjon 2.1.)
...
Elementene heter `vannforekomst`, `kommune`, `fylke`, `landsdel` og/eller `Norge`, avhengig av argumentet `rapportenhet`. 
Hvert av elementene er en tredimensjonal tabell (_array_) med _E_ × _A_ × _S_ tallverdier, der 
_E_ er antall administrative enheter (dimensjonen heter henholdsvis `vannforekomst`, `kommune`, `fylke`, `landsdel` eller `rike`, med verdier tilsvarende kommunenavnene osv.), 
...
I tilegg har de enkelte listeelementene (fra versjon 2.1) et attributt som heter `areal` og er en **numerisk tabell** som inneholder én kolonne for økosystemets relevante areal i km² (`samla`), og én kolonne for hver rapportperiode som angir arealet som det foreligger målinger fra i denne perioden (`maalt.` + årstall). Det er én rad per administrativ enhet. For elver er det angitt lengde i km istedenfor areal.

Videre har listelementene `vannforekomst`, `kommune` og `fylke` et attributt som heter `maalt` og er en **sannhetsverdi-tabell** som inneholder én kolonne for hver rapportperiode som angir om det forelå målinger for samtlige vannforekomster i den aktuelle administrative enheten og rapportperioden. Det er én rad per administrativ enhet.
