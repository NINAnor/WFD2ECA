...
Dataflyt til økologiske tilstandsregnskap fungerer på tilsvarende måte (for detaljert dokumentasjon henvises her til [ecRxiv](https://github.com/NINAnor/ecRxiv)).
...
(2) Filer over vannforekomstenes typologi må lastes ned som excel-filer (csv) fra [vann-nett](https://vann-nett.no/portal/). For øyeblikket (august 2025) er ikke de nødvendige filene tilgjengelig via vann-netts eksportfunksjon. Koden er derfor basert på datafiler som ble lasta ned i mars 2024. For at filene kan leses inn, må de gis følgende navn:
...
I tillegg trenger man en tabell som forklarer kolonnenavna i fila fra NVE.
...
Filnavnet oppgis som argument når dataene leses inn i **R** ved hjelp av funksjonen [`lesInnsjodatabasen`](forklar/lesInnsjodatabasen.md):
```{r, eval=FALSE, echo=TRUE}
...
Informasjon om vannlokaliteter kan leses inn autmatisert via [vannmiljø-API-en](https://vannmiljoapi.miljodirektoratet.no/swagger/ui/index#/Public) eller lastes opp manuelt fra [vannmiljø](https://vannmiljo.miljodirektoratet.no/#/searchwaterlocations). 
Her illustreres innlesing fra API-en. 
...
De relevante vannkategoriene ("L" for innsjø, "R" for elv og/eller "C" for kyst) oppgis som argument når vannlokalitene leses inn i R ved hjelp av funksjonen [`lesVannlokaliteter`](forklar/lesVannlokaliteter.md):
```{r}
...
Denne informasjonen leses inn automatisk, gitt at den er lagra i excel-regneark som heter henholdsvis "**knr.xlsx**", "**fnr.xlsx**", "**VM-param.xlsx**" og "**VM-aktiv.xlsx**", og at disse er plassert i mappa "[data](data/)".
...
Målingene kan også leses inn autmatisert via [vannmiljø-API-en](https://vannmiljoapi.miljodirektoratet.no/swagger/ui/index#/Public) eller lastes opp manuelt fra [vannmiljø](https://vannmiljo.miljodirektoratet.no/#/searchregistrations). 
Funksjonen som står for innlesinga, forutsetter at det foreligger en tabell som forklarer kolonnenavna i fila fra vannmiljø.
...
Funksjonen som leser inn målingene, heter [`lesMaalinger`](forklar/lesMaalinger.md). 
Den leser inn målingene via API når Vannmiljø-databasens forkortelse for parameteren oppgis som argument (f.eks. "ASPT"). 
Når det oppgis filnavnet på den manuelt nedlasta excelfila, leses målingene inn fra denne. 
Det er det siste som illustreres her:
```{r}
...
- [tilpasse en modell](forklar/modell.md) som forklarer variasjonen i måleverdier med tidsperiode, typologifaktorer og [overvåkingsaktivitet](forklar/aktiv.md),
- [ekstrapolere](forklar/extrapol.md) trolige verdier til vannforekomster som det ikke foreligger målinger fra,
- simulere usikkerheten (sannsynlighetsfordelinga) for de sistnevnte og
- [aggregrere](forklar/arealvekt.md) resultatene opp til de ønska administrative enhetene.
...
De resterende argumentene, inkludert mange som ikke vises i eksempelkjøringa under, trenger man bare å oppgi om man ønsker å endre på standardinnstillingene (som er [forklart her](forklar/fraVFtilNI.md)).
...
Noen relevante tester gjennomføres av funksjonen [`oppdaterNImedVF`](forklar/oppdaterNImedVF.R). 
